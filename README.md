...
confidence-tagged **knowledge bundle** on disk — ready for downstream
analytical pipelines, review by subject-matter experts, and consumption
by agentic applications.

The source data is never copied, moved, or modified. The accelerator
reads structure and a small sample of values, and writes a folder of
paired markdown + JSON files describing what it found.
...
| `client-provided`              | Loaded from a client-supplied dictionary (Upcoming, M5).                                  |
| `client-provided-reconciled`   | Reconciled against discovered evidence (Upcoming, M5).                                    |
| `ai-drafted`                   | Drafted automatically; awaiting reviewer edit (Upcoming, M3).                             |
| `ai-drafted-edited`            | Reviewer edited an automated draft (Upcoming, M3+).                                       |
